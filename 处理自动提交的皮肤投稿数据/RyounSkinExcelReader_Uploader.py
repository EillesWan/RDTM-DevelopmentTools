...
for line in lines:# type: ignore
    ...
    if line[0]:
        ...
        iconRSPSdata, iconSIGN = upPic(f"{projectDirection}渲染图TA_生成000.png")
        if not (iconRSPSdata, iconSIGN):
            allProjrcts['警告'].append(
                {
                    "name": projectName,
                    "author": author,
                    "status": '警告',
                    "msg": f'[本地]无法上传图片{projectDirection}渲染图TA_生成000.png',
                    "DEAL": (projectCost, cost_) if CostErr else False,
                }
            )
            prt("已经使用已有图片")
            iconRSPSdata = '{"url": "https://x19.fp.ps.netease.com/file/6281287e2ef6ee847a2806d29bOEEKqu04", "mime": "image/png; charset=binary", "fsize": 423480, "md5": "acda3ea7a04a7348b3c8f800cb93307a", "picSize": [1000, 1000]}'
            iconSIGN = "h7x9mkkslt7o5tYs1vS4RgRvCpY="

            
        showRSPSdata, showSIGN = upPic(f"{projectDirection}渲染图TB_生成000.png")
        if not (showRSPSdata, showSIGN):
            allProjrcts['警告'].append(
                {
                    "name": projectName,
                    "author": author,
                    "status": '警告',
                    "msg": f'[本地]无法上传图片{projectDirection}渲染图TB_生成000.png',
                    "DEAL": (projectCost, cost_) if CostErr else False,
                }
            )
            prt("已经使用已有图片")
            showRSPSdata = '{"url": "https://x19.fp.ps.netease.com/file/628128822ef6ee9b09f0372fEs51Kqk504", "mime": "image/png; charset=binary", "fsize": 346267, "md5": "c4cccee7ed2d50f9f075d5116f1707f8", "picSize": [900, 580]}'
            showSIGN = "9Ygz7oDrmnEsvh9t6+gAPETcTvs="
        ...
        datadict = {
            ...
            "pc_pri_type": None,
            "pc_sub_type": None,
            "info": f'<p><img src="https://x19.fp.ps.netease.com/file/627020d4442e29577c303660uydAI6HY04"></p><p><span class="ql-size-large">本组件作者：</span><strong class="ql-size-huge" style="color: blue;">{author}</strong>{isMember_}</p><p>{projectDescription}<p><br /></p><p class="ql-align-center"><strong style="color: blue" class="ql-size-huge">凌天之云创新我的世界开发团队</strong></p><p class="ql-align-center"><span style="color: blue" class="ql-size-large">Ryoun Development Team for Minecraft</span></p><p class="ql-align-center"><strong style="color: orange" class="ql-size-large">深挚同心 共创遥途</strong></p><p><span class="ql-size-large">欢迎投稿您的各种优秀作品！无论是皮肤作品、玩法组件、游戏地图还是材质光影，都可以为大家看到，发出它的闪光点！</span></p><p><em class="ql-size-large">若要前来探讨您在游玩时遇到的问题以及提供信息反馈，或者提出疑问、代发资源请加：</em><strong class="ql-size-large"><em>946419613</em></strong></p>',  # 详情简介
            "pc_info": "",
            "rarity": 0,
            "lobby_min_num": 0,
            "lobby_max_num": 0,
