...
cookie = open(input('Cookie文件: '),'r').read()
...
aGETurl = 'https://mc-launcher.webapp.163.com/filepicker/token?file_type=png&secure=false'
...
for line in lines:
    ...
    if line[0]:
        ...
        isMember = '【非本团队成员】' if line[4] == '以上皆非' else ('【星河团队成员】' if '星河' in line[4] else ('【ICW团队成员】' if 'ICW' in line[4] else ('【凌创工作室成员】' if '工作室' in line[4] else '')))
        ...
        for i in skin:
            if i:
                ...
                try:
                    ...
                    picturePath = f'{projectDirection}{n}'.replace(n[: n.index('.')], f'展开图{skin.index(i)}')
        ...
        boundary = '---------------------------'+''.join([str(random.randint(0, 9)) for i in range(29)])
...
""".encode()+open(picturePath, 'rb',).read()+f"""
--{boundary}--""".encode()
        ...
        print('====================开始投稿资源：提交资源信息')
        isMember = f'<strong style="color: green;" class="ql-size-small">{isMember}</strong>' if isMember else ""
        datadict = {
            ...
            'sub_type': 13 if projectType == '原版' else (14 if projectType == '科幻' else (15 if projectType == '神话' else 16)), # 上传组件子类型 地图-闯关1 RPG2 PVP3 建筑4 其他5 | 模组-玩法6 道具7 生物8 其他9 一键生成10 | 材质光影-材质11 光影12 | 皮肤-原版13 科幻14 神话15 其他16 | 联机大厅- 未考证
            'pc_pri_type': None,
            'pc_sub_type': None,
        ...
        input(f'失败：{result["msg"]}' if result["status"] == 'fail' else ( f'成功\n组件ID：{result["data"]["item_id"]}' if result["status"] == 'ok' else f'未知{result}'))
